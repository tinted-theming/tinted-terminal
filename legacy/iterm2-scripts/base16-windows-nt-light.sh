#!/bin/sh
# iTerm2 Terminal Script Theme
# Scheme system: base16
# Scheme name: Windows NT Light (legacy)
# Scheme author: Fergus Collins (https://github.com/C-Fergus)
# Template author: Tinted Theming (https://github.com/tinted-theming/tinted-terminal)

osascript <<EOF
tell application "iTerm2"
    tell current session of current window
        set background color to {65535, 65535, 65535}
        set foreground color to {32896, 32896, 32896}

        -- Set ANSI Colors
        set ANSI black color to {60138, 60138, 60138}
        set ANSI red color to {32896, 0, 0}
        set ANSI green color to {0, 32896, 0}
        set ANSI yellow color to {32896, 32896, 0}
        set ANSI blue color to {0, 0, 32896}
        set ANSI magenta color to {32896, 0, 32896}
        set ANSI cyan color to {0, 32896, 32896}
        set ANSI white color to {16448, 16448, 16448}

        -- Set Bright ANSI Colors
        set ANSI bright black color to {54741, 54741, 54741}
        set ANSI bright red color to {65535, 65535, 0}
        set ANSI bright green color to {60138, 60138, 60138}
        set ANSI bright yellow color to {54741, 54741, 54741}
        set ANSI bright blue color to {41120, 41120, 41120}
        set ANSI bright magenta color to {16448, 16448, 16448}
        set ANSI bright cyan color to {0, 65535, 0}
        set ANSI bright white color to {0, 0, 0}
    end tell
end tell
EOF
