#!/bin/sh
# iTerm2 Terminal Script Theme
# Scheme system: base16
# Scheme name: darkmoss (legacy)
# Scheme author: Gabriel Avanzi (https://github.com/avanzzzi)
# Template author: Tinted Theming (https://github.com/tinted-theming/tinted-terminal)

osascript <<EOF
tell application "iTerm2"
    tell current session of current window
        set background color to {5911, 7710, 7967}
        set foreground color to {51143, 51143, 42405}

        -- Set ANSI Colors
        set ANSI black color to {9509, 11308, 11565}
        set ANSI red color to {65535, 17990, 22616}
        set ANSI green color to {18761, 37265, 32896}
        set ANSI yellow color to {65021, 45489, 7967}
        set ANSI blue color to {18761, 32896, 37265}
        set ANSI magenta color to {39835, 49344, 51400}
        set ANSI cyan color to {26214, 55769, 61423}
        set ANSI white color to {58339, 58339, 51400}

        -- Set Bright ANSI Colors
        set ANSI bright black color to {14135, 15420, 15677}
        set ANSI bright red color to {59110, 56283, 29812}
        set ANSI bright green color to {9509, 11308, 11565}
        set ANSI bright yellow color to {14135, 15420, 15677}
        set ANSI bright blue color to {33153, 36751, 32896}
        set ANSI bright magenta color to {58339, 58339, 51400}
        set ANSI bright cyan color to {53970, 31611, 21331}
        set ANSI bright white color to {57825, 60138, 61423}
    end tell
end tell
EOF
